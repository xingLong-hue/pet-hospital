<template>
	<div>
		...
		<el-card>
			<el-row :gutter="20">
				<el-col :span="9">
					<el-input placeholder="请输入用户名搜索" v-model='queryInfo.query_data' clearable @clear="getuser">
			...
			<!-- 添加用户的对话框 -->
			<el-dialog title="用户信息" :visible.sync="AddDialogVisible" width="50%" @close="AddDialogClose">
	...
	export default {
		...
		methods:{
			...
			async getuser(){
				...
				let {data:res} = await this.$http.get('doctor/doctoruser',{params:this.queryInfo})
				// console.log(res.data)
				this.total=res.paging.total
				window.sessionStorage.setItem("doctorUser_total",this.total)
			...
			async delete_user(id) {
				...
				const {data:res} = await this.$http.delete("doctor/delete_doctor/",{data:userid})
			...
			sure_edit(edit_form) {
				...
				this.$refs.edit_formRef.validate(async (vaild) => {
					...
					if (!vaild) {
						...
						const {
							data: res
						} = await this.$http.put('doctor/update_doctor',data)
			...
			async sure_add(add_form_data){
				...
				this.$refs.Add_formRef.validate(async (vaild) => {
					if(!vaild){
						...
						// console.log(datamsg)
						let {data:res} = await this.$http.post('doctor/add_doctor',datamsg)
