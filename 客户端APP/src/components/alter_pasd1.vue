		...
		<div class="box">
			...
			<van-form @submit="onSubmit(form)">
				...
				<van-cell-group inset>
					<van-field v-model="form.password1" name="oldpasd" label="原来的密码" placeholder="原来的密码"
						:rules="[{ required: true, message: '请填写密码' }]" />
					<van-field v-model="form.password2" type="password" name="pasd" label="新的密码" placeholder="密码"
						:rules="[{ required: true, message: '请填写密码' }]" autocomplete/>
					<van-field v-model="form.password3" type="password" name="pasd2" label="再次输入密码" placeholder="密码"
						:rules="[{ required: true, message: '请填写密码' }]" />
				</van-cell-group>
				...
				if (form.password2 !== form.password3) {
			     ...
			     // 修改前验证原来的密码是否正确
					...
					if(res.status==200){
						...
						let {
							data: res
						} = await this.$http.post('/doctor/update_pasd',da)
						console.log(res)
						if(res.status==200){
							Toast('修改密码成功');
							this.$router.push('/')
